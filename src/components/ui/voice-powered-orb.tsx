...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const initMicrophone = async () => {
    try {
      ...
      analyserRef.current.smoothingTimeConstant = 0.8; // More smoothing for fluid motion
    ...
    try {
      ...
      const update = (t: number) => {
        ...
        if (enableVoiceControl && isMicrophoneInitialized) {
          targetVoiceLevel = analyzeAudio();

          // Smooth interpolation for fluid motion
          const lerpFactor = 1 - Math.pow(0.001, dt);
          voiceLevel = voiceLevel + (targetVoiceLevel - voiceLevel) * lerpFactor;
          ...
          targetHoverIntensity = Math.min(
            voiceLevel * maxHoverIntensity * 0.8,
            maxHoverIntensity
          );

          program.uniforms.hover.value += (targetHover - program.uniforms.hover.value) * lerpFactor;
          program.uniforms.hoverIntensity.value += (targetHoverIntensity - program.uniforms.hoverIntensity.value) * lerpFactor;
        } else {
          // Smooth fade out when not using voice control
          const lerpFactor = 1 - Math.pow(0.001, dt);
          program.uniforms.hover.value += (0 - program.uniforms.hover.value) * lerpFactor;
          program.uniforms.hoverIntensity.value += (0 - program.uniforms.hoverIntensity.value) * lerpFactor;
          voiceLevel += (0 - voiceLevel) * lerpFactor;
  ...
  return <div ref={ctnDom} className={cn("w-full h-full relative", className)}></div>;
};
